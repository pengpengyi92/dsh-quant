...
test('real Loader composition: 21 tools register and are model-visible', async () => {
  const ctx = await boot()
  const names = ctx.tools.schemas().map(s => s.name).sort()
  assert.deepEqual(names, [
    'quant_adx', 'quant_atr', 'quant_backtest', 'quant_backtest_bollinger',
    'quant_backtest_grid', 'quant_backtest_portfolio', 'quant_backtest_rsi', 'quant_bollinger', 'quant_cci', 'quant_data_advice', 'quant_data_compare', 'quant_data_guide', 'quant_ema',
    'quant_kdj', 'quant_macd', 'quant_market_fetch', 'quant_obv', 'quant_roc', 'quant_rsi',
...
test('real Loader composition: disposal removes the registered tools (HMR-safety)', async () => {
  const ctx = await boot()
  assert.equal(ctx.tools.schemas().length, 21)
  ...
  assert.equal(ctx.tools.schemas().length, 22)
  disposer()
  assert.equal(ctx.tools.schemas().length, 21)
})
