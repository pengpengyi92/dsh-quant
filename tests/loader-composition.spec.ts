...
test('real Loader composition: 50 tools register and are model-visible', async () => {
  ...
  assert.deepEqual(names, [
    ...
    'quant_kdj', 'quant_linear_model', 'quant_macd', 'quant_market_fetch', 'quant_metrics', 'quant_npm_stats', 'quant_obv', 'quant_option', 'quant_oss_pulse', 'quant_portfolio_optimize', 'quant_repo_stats', 'quant_report', 'quant_resample', 'quant_research_pipeline', 'quant_risk', 'quant_roc', 'quant_rsi', 'quant_series_quality', 'quant_series_stats',
    'quant_sma', 'quant_var_backtest', 'quant_volatility', 'quant_walk_forward', 'quant_williams_r',
  ])
...
test('real Loader composition: disposal removes the registered tools (HMR-safety)', async () => {
  const ctx = await boot()
  assert.equal(ctx.tools.schemas().length, 50)
  ...
  assert.equal(ctx.tools.schemas().length, 51)
  disposer()
  assert.equal(ctx.tools.schemas().length, 50)
})
