...
test('real Loader composition: 53 tools register and are model-visible', async () => {
  ...
  assert.deepEqual(names, [
    ...
    'quant_backtest_grid', 'quant_backtest_portfolio', 'quant_backtest_rsi', 'quant_bollinger', 'quant_bond', 'quant_cci', 'quant_channel_guide', 'quant_chart', 'quant_data_advice', 'quant_data_annotate', 'quant_data_compare', 'quant_data_guide', 'quant_data_pit', 'quant_data_quality', 'quant_drawdown', 'quant_ema', 'quant_execute_sim', 'quant_factor_combine', 'quant_factor_evaluate', 'quant_factor_neutralize', 'quant_fund', 'quant_ic_decay',
    'quant_kdj', 'quant_layered_backtest', 'quant_linear_model', 'quant_macd', 'quant_market_fetch', 'quant_metrics', 'quant_npm_stats', 'quant_obv', 'quant_option', 'quant_oss_pulse', 'quant_portfolio_optimize', 'quant_repo_stats', 'quant_report', 'quant_resample', 'quant_research_pipeline', 'quant_risk', 'quant_roc', 'quant_rsi', 'quant_series_quality', 'quant_series_stats',
    'quant_sma', 'quant_trade_quality', 'quant_var_backtest', 'quant_volatility', 'quant_walk_forward', 'quant_williams_r',
  ])
...
test('real Loader composition: disposal removes the registered tools (HMR-safety)', async () => {
  const ctx = await boot()
  assert.equal(ctx.tools.schemas().length, 53)
  ...
  assert.equal(ctx.tools.schemas().length, 54)
  disposer()
  assert.equal(ctx.tools.schemas().length, 53)
})
