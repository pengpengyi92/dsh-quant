...
test('real Loader composition: 28 tools register and are model-visible', async () => {
  ...
  assert.deepEqual(names, [
    ...
    'quant_kdj', 'quant_macd', 'quant_market_fetch', 'quant_obv', 'quant_roc', 'quant_rsi', 'quant_series_quality', 'quant_series_stats',
...
test('real Loader composition: disposal removes the registered tools (HMR-safety)', async () => {
  const ctx = await boot()
  assert.equal(ctx.tools.schemas().length, 28)
