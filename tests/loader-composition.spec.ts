...
async function boot(extraLines: readonly string[] = []): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['dsh-quant-indicators', QuantIndicators],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
test('real Loader composition: 18 tools register and are model-visible', async () => {
  const ctx = await boot()
  const names = ctx.tools.schemas().map(s => s.name).sort()
  assert.deepEqual(names, [
    'quant_adx', 'quant_atr', 'quant_backtest', 'quant_backtest_bollinger',
    'quant_backtest_grid', 'quant_backtest_portfolio', 'quant_backtest_rsi', 'quant_bollinger', 'quant_cci', 'quant_ema',
...
test('real Loader composition: disposal removes the registered tools (HMR-safety)', async () => {
  const ctx = await boot()
  assert.equal(ctx.tools.schemas().length, 18)
  ...
  assert.equal(ctx.tools.schemas().length, 19)
  disposer()
  assert.equal(ctx.tools.schemas().length, 18)
})
